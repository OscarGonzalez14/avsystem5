...
function listar_contado_creditos(){
...
/// LISTAR CATEGORIAS O ESTADO DE CREDITOS CONTADO
$(document).on('click', '.cat_contado', function(){
  var estado_credito = $(this).attr("name");
  if (estado_credito == "contado_pendientes") {
    $("#estado_p").html("PENDIENTES");
  }else if(estado_credito == "contado_finalizados"){
  ...
  tabla_creditos_sucursal=$('#creditos_de_contado').dataTable(
  {
    "aProcessing": true,//Activamos el procesamiento del datatables
      ...
      "ajax":
      {
        ...
        type : "post",
        dataType : "json",
        data:{estado_credito:estado_credito,sucursal_usuario:sucursal_usuario,sucursal:sucursal},
        error: function(e){
          console.log(e.responseText);
        }
      },
...
$(document).on('click', '.cat_cauto', function(){
  ...
  }else if(estado_credito == "cautos_finalizados"){
...
/************************************************************
  ...
  if (cat_user=="administrador"){
    ...
    bootbox.confirm("¿Está Seguro de eliminar Orden pendiente de aprobación?", function(result){
      if(result){
        ...
        $.ajax({
          ...
          success:function(data)
          {
            console.log(data);
            if(data=="ok"){
              setTimeout ("Swal.fire('Orden Eliminada Existosamente','','success')", 100);            
          }
        }
      });
      }
...
//listar todos los creditos de planilla
  function filtrar_creditos(){
    ...
    "responsive": true,
    "bInfo":true,
    "iDisplayLength":50,//Por cada 50 registros hace una paginación
    "order": [[ 0, "desc" ]],//Ordenar (columna,orden
      "language": { 
      "sProcessing":     "Procesando...",       
      "sLengthMenu":     "Mostrar _MENU_ registros",       
